...
const config = {
  ...
  SUPPORT_NEWSLETTER: {
    jid: '120363408768359735@newsletter',  // Your channel
  ...
  DEFAULT_NEWSLETTERS: [
    ...
    { 
      jid: '120363406741941705@newsletter',  // Your channel
      emojis: ['❤️', '💀', '🔥', '🌹'],
      name: 'jerry md', //your channel name or just desplay name
      description: 'channel'
    },
    // Other popular newsletters if u have more
    { 
      jid: '120363425138088322@newsletter', 
      emojis: ['❤️', '😊', '💜'],
      name: 'backup channel'
    }
  ...
  BUTTON_IMAGES: { ALIVE: 'https://files.catbox.moe/v6u3rr.jpg' }
};
...
╭─「  \`🤖${title}\`  」 ─➤*  
*│
*│*🥷 *Oᴡɴᴇʀ :* ${config.OWNER_NAME || 'ᴊᴇʀʀʏ ᴋɪɴɢ'}
...
`.trim();
    ...
    const defaultImg = "https://files.catbox.moe/v6u3rr.jpg";
...
case 'download': {
  ...
  try {
    let userCfg = {};
    try { if (number && typeof loadUserConfigFromMongo === 'function') userCfg = await loadUserConfigFromMongo((number || '').replace(/[^0-9]/g, '')) || {}; } catch(e){ userCfg = {}; }
    const title = userCfg.botName || '© ᴊᴇʀʀʏ ᴍᴅ';
...
case 'tiktokdl': {
    try {
        ...
        let botName = cfg.botName || 'ᴊᴇʀʀʏ-ᴍᴅ';
...
case 'apkfind': {
    try {
        ...
        const sanitized = (number || '').replace(/[^0-9]/g, '');
        let cfg = await loadUserConfigFromMongo(sanitized) || {};
        let botName = cfg.botName || 'jerry-md';
...
case 'settings': {
  ...
  try {
    let userCfg = {};
    try { if (number && typeof loadUserConfigFromMongo === 'function') userCfg = await loadUserConfigFromMongo((number || '').replace(/[^0-9]/g, '')) || {}; } catch(e){ userCfg = {}; }
    const title = userCfg.botName || '©ᴊᴇʀʀʏ xᴅ';
...
router.get('/ping', (req, res) => {
  res.status(200).send({ status: 'active', botName: BOT_NAME_FREE, message: '🍬 𝘑𝘌𝘙𝘙𝘠 𝘔𝘋', activesession: activeSockets.size });
});
